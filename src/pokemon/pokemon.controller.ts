...
export class PokemonController {
    ...
    async getByIdOrName(@Param('id_or_name') nameOrId: string) {
        ...
        if (!results.length) {
            throw new HttpException('Pokemon with given name or id does not exist in database.', HttpStatus.BAD_REQUEST);
        }
    ...
    async getByType(@Param('type') type: string) {
        ...
        if (!results.length) {
            throw new HttpException('Pokemon with given type does not exist in database.', HttpStatus.BAD_REQUEST);
        }
